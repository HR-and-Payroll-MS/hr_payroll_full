...
class PayslipSerializer(serializers.ModelSerializer):
    ...
    def get_bank_account(self, obj):
        ...
        # Determine access: only Managers/Admins or the employee themselves (or superuser) may see full account
        can_view_full = False
        if request and getattr(request, 'user', None) and request.user.is_authenticated:
            ...
            if user.is_superuser:
                can_view_full = True
            # Manager/Admin group
            if user.groups.filter(name__iexact='Manager').exists() or user.groups.filter(name__iexact='Admin').exists():
                can_view_full = True
            # Employee owner
