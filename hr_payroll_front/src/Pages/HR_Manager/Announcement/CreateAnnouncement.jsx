...
export default function CreateAnnouncement({
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      toast.show({ type: 'error', message: 'Failed to save announcement.' });
    }
  };
            ...
            <h2 className="font-bold text-xl text-slate-800 dark:text-white uppercase tracking-tight">
              {initialData
                ? step === 1
                  ...
                  : 'Review Updates'
                : step === 1
                  ? 'New Internal Announcement'
                  : 'Review Publication'}
            </h2>
            <p className="text-[10px] uppercase font-bold text-slate-400">
              {step === 1
                ? 'Configure communication details'
                : 'Final view before employee broadcast'}
            </p>
          </div>
          <button
            onClick={resetForm}
            ...
            <div className="flex flex-col gap-6">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] font-black text-slate-500 dark:text-slate-300 uppercase tracking-widest">
                    Priority
                  </label>
                  <Dropdown
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
                  {attachments.map((f, i) => (
                    <div
                      key={i}
                      className="relative aspect-square bg-slate-200 dark:bg-slate-800 rounded border dark:border-slate-500 overflow-hidden group"
                    >
                      {f.type === 'image' ? (
                        <img
                          src={f.url}
                        ...
                        <div className="flex items-center justify-center h-full text-[9px] font-bold text-slate-500 p-2 text-center break-all uppercase">
                          {f.name}
                        </div>
                      )}
                      <button
                        ...
                        className="absolute top-1 right-1 bg-red-600 text-white rounded-full w-5 h-5 flex items-center justify-center opacity-0 group-hover:opacity-100 transition shadow-md"
                      >
                        ×
                      </button>
                    </div>
                ...
                {initialData && (
                  <p className="text-[10px] text-slate-400 italic mt-1 font-bold">
                    Existing attachments are preserved. Only new files will be
                    uploaded.
                  </p>
                )}
              </div>
        ...
        <div className="p-5 border-t border-slate-200 dark:border-slate-600 flex gap-4 bg-white dark:bg-slate-800">
          {step === 1 ? (
            ...
            <>
              <button
                onClick={() => setStep(1)}
                className="px-6 py-3 bg-slate-100 dark:bg-slate-600 dark:text-white font-bold rounded uppercase text-xs"
              >
                Back to Edit
              </button>
              <button
                onClick={handlePublish}
                className="ml-auto bg-green-700 text-white font-bold px-10 py-3 rounded shadow-lg uppercase text-xs tracking-widest hover:bg-green-800 transition active:scale-95"
              >
