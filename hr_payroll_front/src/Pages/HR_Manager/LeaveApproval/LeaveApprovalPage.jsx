...
export default function LeaveApprovalPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        const [requestsRes, employeesRes] = await Promise.all([
          axiosPrivate.get('/leaves/requests/'),
          axiosPrivate.get('/employees/')
        ]);
        
        // Transform backend data to match frontend expected format
        const transformedRequests = (requestsRes.data.results || requestsRes.data || []).map(req => ({
          id: req.id,
        ...
        const transformedEmployees = (employeesRes.data.results || employeesRes.data || []).map(emp => ({
          id: emp.id,
          name: emp.fullname || `${emp.general?.firstname} ${emp.general?.lastname}`,
          dept: emp.department || emp.job?.department || 'N/A',
  ...
  async function handleDecision(reqId, decision, comment) {
    try {
      ...
      setDrawerOpen(false);
      pushToast(decision === 'approved' ? 'Leave Request Approved' : 'Leave Request Denied');
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="h-full w-full flex items-center justify-center bg-white dark:bg-slate-800">
        <div className="text-slate-500 dark:text-slate-400">Loading leave requests...</div>
      </div>
    );
  }
