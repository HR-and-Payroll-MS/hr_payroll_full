...
import { User, Calendar, FileText, GitBranch, MessageSquare, Info, Paperclip, Download, Eye } from 'lucide-react';
...
export default function RequestDetails({ req, employees }) {
  ...
  const emp = employees.find(e => e.id === (req.employee || req.employeeId)) || {
  ...
  const photoUrl = emp.photo 
    ? (emp.photo.startsWith('http') ? emp.photo : `${import.meta.env.VITE_BASE_URL || import.meta.env.VITE_API_URL || ''}${emp.photo}`)
    : null;
  ...
  return (
    <div className="h-full scrollbar-hidden w-full flex flex-col gap-4 text-slate-800 dark:text-slate-100 relative">
      ...
      <div className="bg-gray-50 dark:bg-slate-700 p-4 rounded shadow dark:shadow-black dark:inset-shadow-xs dark:inset-shadow-slate-600 flex items-center gap-4 transition-colors">
        ...
        <div className={`h-14 w-14 rounded-full flex items-center justify-center overflow-hidden text-white font-bold text-lg shadow-sm border-2 border-white dark:border-slate-500 ${!photoUrl ? emp.avatarColor : 'bg-white'}`}>
          {photoUrl ? (
      ...
      <div className="bg-gray-50 dark:bg-slate-700 p-4 rounded shadow dark:shadow-black dark:inset-shadow-xs dark:inset-shadow-slate-600">
        ...
        <div className="space-y-4 ml-2">
          {req.approvalChain.map((step, idx) => (
            <div key={idx} className="relative pl-6 border-l-2 border-slate-200 dark:border-slate-600 last:border-l-0">
