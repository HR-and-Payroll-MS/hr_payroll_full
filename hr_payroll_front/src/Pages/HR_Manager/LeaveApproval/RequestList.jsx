...
export default function RequestList({ requests, employees, filter, setFilter, onOpen }) {
  ...
  <div className="flex-1 p-2 space-y-3 overflow-auto scrollbar-hidden">
    {requests.map(req => {
      const emp = employees.find(e => e.id === req.employeeId) || { name: 'Unknown', dept: 'N/A' };
      ...
      return (
        ...
        >
          ...
          <div className="flex justify-between items-start pl-2 gap-3">
            ...
            <div className={`h-10 w-10 rounded-full flex items-center justify-center overflow-hidden shrink-0 border border-slate-200 dark:border-slate-600 ${!emp.photo ? 'bg-indigo-100 text-indigo-600' : 'bg-white'}`}>
                {emp.photo ? (
                    <img 
                    src={emp.photo.startsWith('http') ? emp.photo : `${import.meta.env.VITE_BASE_URL || import.meta.env.VITE_API_URL || ''}${emp.photo}`} 
                    alt={emp.name} 
