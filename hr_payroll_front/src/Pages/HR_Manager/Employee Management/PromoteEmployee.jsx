...
import { ArrowUp, ArrowDown, Users, Building, Briefcase, CheckCircle, XCircle } from "lucide-react";
...
const PromoteEmployee = () => {
    ...
    const handlePromote = async () => {
        if (!selectedEmployee) {
            setMessage({ type: 'error', text: 'Please select an employee to promote' });
            return;
        }
        ...
        try {
            ...
            setMessage({ type: 'success', text: response.data.message || 'Employee promoted successfully!' });
            setActionType(null);
            setSelectedEmployee(null);
    ...
    const handleDemote = async () => {
        if (!selectedEmployee) {
            setMessage({ type: 'error', text: 'Please select an employee to demote' });
            return;
        }
        ...
        try {
            ...
            setMessage({ type: 'success', text: response.data.message || 'Employee demoted successfully!' });
            setActionType(null);
            setSelectedEmployee(null);
        ...
        <div className="w-full h-full p-6 overflow-y-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {selectedEmployee && (
                        <div className="bg-slate-50 dark:bg-slate-700/50 rounded-lg p-4">
                            <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">Current Details</p>
                            <p className="font-medium text-slate-700 dark:text-slate-200">
                                {selectedEmployee.label}
                            </p>
                            <p className="text-sm text-slate-500 dark:text-slate-400">
                                {selectedEmployee.jobtitle || 'No title'} • {selectedEmployee.department || 'No department'}
                            </p>
                        </div>
                    )}
                </div>
            ...
            {selectedEmployee && (
                <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg dark:shadow-black/20 p-6 mb-6">
                    ...
                    <div className="flex flex-wrap gap-4">
                        ...
                        >
                            <ArrowUp size={24} className={actionType === 'promote' ? 'text-green-500' : 'text-slate-400'} />
                        ...
                        >
                            <ArrowDown size={24} className={actionType === 'demote' ? 'text-amber-500' : 'text-slate-400'} />
                            <div className="text-left">
            ...
            {actionType && (
                <div className="flex justify-end">
                    <button
                        onClick={actionType === 'promote' ? handlePromote : handleDemote}
                        disabled={loading}
                        className={`px-8 py-3 rounded-xl font-semibold text-white transition-all flex items-center gap-2 ${
                            actionType === 'promote'
                                ...
                                : 'bg-amber-600 hover:bg-amber-700 shadow-lg shadow-amber-200 dark:shadow-none'
                        } disabled:opacity-50`}
                    >
                        {loading ? (
                            <ThreeDots />
                            ...
                            <>
                                {actionType === 'promote' ? <ArrowUp size={18} /> : <ArrowDown size={18} />}
                                {actionType === 'promote' ? 'Promote Employee' : 'Demote Employee'}
                            </>
                        )}
                    </button>
                </div>
