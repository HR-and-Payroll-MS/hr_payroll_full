...
function ViewEmployeeDetail() {
  ...
  useEffect(() => {
     axiosPrivate.get('/attendances/schedules/')
       .then(res => {
  ...
  useEffect(() => {
    const fetchEmployee = async () => {
      try {
         ...
         if (empData.job) {
             const wsKey = Object.keys(empData.job).find(k => k.toLowerCase() === 'workschedule' || k.toLowerCase() === 'work_schedule');
             if (wsKey && typeof empData.job[wsKey] === 'object' && empData.job[wsKey] !== null) {
                 empData.job.work_schedule = empData.job[wsKey].title || empData.job[wsKey].name;
    ...
    setEmployeeData((prev) => {
        ...
        if (section === 'job' && field.toLowerCase().includes('department')) {
            ...
            if (dept && dept.manager) {
                 ...
                 if (mgr) {
                     const mgrName = mgr.fullname || `${mgr.general?.firstname} ${mgr.general?.lastname}`;
                     // Find the manager key in the job section
                     const lmKey = Object.keys(prev[section]).find(k => k.toLowerCase().includes('manager')) || 'line_manager';
  ...
  const handleSave = async (section) => {
    try {
      ...
      // Special handling for job section (work_schedule mapping)
      if (section === 'job') {
          const jobPayload = { ...payload.job };
          if (jobPayload.work_schedule) {
              const sched = schedules.find(s => s.title === jobPayload.work_schedule);
              if (sched) jobPayload.work_schedule = sched.id;
              else if (jobPayload.work_schedule === "") jobPayload.work_schedule = null;
          } else {
      ...
      if (section !== 'documents') {
          ...
          const res = await axiosPrivate.patch(`/employees/${employeeId}/`, payload);
      ...
      const msg = err.response?.data?.error || err.response?.data?.message || "Failed to save. Try again.";
      setError(msg);
    }
  };
