...
import { Building, UserPlus, CheckCircle, XCircle, Users, ArrowRight, UserCog } from "lucide-react";
...
const AssignToDepartment = () => {
    ...
    useEffect(() => {
        if (selectedDept && selectedDept.managerId && employees.data) {
            const mgr = employees.data.find(e => e.id === selectedDept.managerId);
            if (mgr) {
                // If the employee IS the manager, do not assign themselves as line manager
                if (selectedEmployee && selectedEmployee.id === mgr.id) {
                    setSelectedManager(null);
                } else {
                    setSelectedManager({
                        id: mgr.id,
                        label: mgr.fullname || `${mgr.general?.firstname} ${mgr.general?.lastname}`
                    });
                }
            } else {
                 setSelectedManager(null);
             ...
             // If no manager for dept, or no dept selected, reset (or keep previous? Reset is safer)
             // Unless user manually selected one? For now, reset to be consistent with "Dept's Manager" logic.
             // If we want manual override to persist, we need a flag. But simple is better.
    ...
    const handleEmployeeSelect = (emp) => {
        ...
        setSelectedEmployee({
            id: emp.id,
            label: emp.fullname || `${emp.general?.firstname || ''} ${emp.general?.lastname || ''}`.trim(),
    ...
    const handleManagerSelect = (mgr) => {
         ...
         setSelectedManager({
            id: mgr.id,
            label: mgr.fullname || `${mgr.general?.firstname || ''} ${mgr.general?.lastname || ''}`.trim(),
         });
    };
    
    const handleAssign = async () => {
        if (!selectedEmployee || !selectedDept) {
            setMessage({ type: 'error', text: 'Please select both an employee and a department' });
            return;
        }
                ...
                <div className="bg-white dark:bg-slate-800 rounded-xl shadow dark:shadow-slate-900 dark:shadow-md dark:inset-shadow-xs dark:inset-shadow-slate-600 p-6 space-y-6 h-fit">
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <h2 className="font-semibold text-slate-700 dark:text-slate-200">1. Select Employee</h2>
                        </div>
                        ...
                        {selectedEmployee && (
                            <div className="mt-3 bg-slate-50 dark:bg-slate-700/50 p-3 rounded text-sm">
                                <span className="text-slate-500 dark:text-slate-400">Current Dept: </span>
                                <span className="font-semibold text-slate-700 dark:text-slate-200">{selectedEmployee.department || 'None'}</span>
                            </div>
                        )}
                    </div>
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <h2 className="font-semibold text-slate-700 dark:text-slate-200">2. Select Target Department</h2>
                        </div>
                        <Dropdown
                        ...
                        <div className="relative z-20">
                            ...
                            {selectedManager ? (
                                ...
                                <div className="mt-3 bg-slate-50 dark:bg-slate-700/50 p-3 rounded text-sm italic text-slate-400">
                                    No Line Manager assigned (Report directly to Head of Dept or None)
                                </div>
                            )}
                        </div>
                    </div>
                            ...
                            <div className="w-full space-y-6">
                                <div className="flex items-center justify-between bg-slate-50 dark:bg-slate-700/30 p-4 rounded-xl">
                                    <div className="text-left">
                                        <p className="text-xs text-slate-500 uppercase font-bold">Employee</p>
                                        <p className="font-semibold text-lg">{selectedEmployee.label}</p>
                                    </div>
                                    <ArrowRight className="text-slate-400" />
                                    <div className="text-right">
                                        <p className="text-xs text-slate-500 uppercase font-bold">New Department</p>
                                        <p className="font-semibold text-lg text-purple-600 dark:text-purple-400">{selectedDept.name}</p>
                                    </div>
                                </div>
                                
                                <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-xl text-left">
                                    <p className="text-sm font-semibold text-blue-800 dark:text-blue-300 mb-2">Changes that will apply:</p>
                                    <ul className="text-sm text-blue-700 dark:text-blue-400 space-y-1 list-disc list-inside">
                                        <li>Department will update to <b>{selectedDept.name}</b></li>
                                        <li>Line Manager will be set to: <b>{selectedManager ? selectedManager.label : 'None'}</b></li>
                                    </ul>
                                </div>
