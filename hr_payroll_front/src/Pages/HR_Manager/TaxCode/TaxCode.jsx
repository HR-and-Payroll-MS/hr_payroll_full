...
function TaxCode() {
    ...
    setAllTaxCodes(prev =>
      prev.map(c => {
         ...
         const updatedVersions = !newStatus 
             ? c.versions.map(v => ({ ...v, status: { ...v.status, active: false } }))
             : c.versions;
      ...
      if (versionsToDeactivate.length > 0) {
         await Promise.all(
            versionsToDeactivate.map(v => 
               axiosPrivate.patch(`/payroll/tax-versions/${v.id}/`, { is_active: false })
            )
         );
      }
  ...
  const saveConfiguration = async () => {
    try {
      ...
      const payload = {
         code: selectedCodeId || `TX_${taxData.name.toUpperCase().slice(0, 3)}_${Date.now().toString().slice(-4)}`,
         name: taxData.name,
         is_active: true,
         // Version data
         ...
         tax_brackets: taxData.incomeTax.brackets?.map(b => ({
           min_income: b.min,
      ...
      if (selectedCodeId) {
         ...
         response = await axiosPrivate.post(`/payroll/tax-codes/${taxCodeObj.backendId}/add-version/`, payload);
      } else {
         // NEW TAX CODE
         ...
         // Fix: If backend doesn't handle nested version creation, we must create it explicitly
             if (response.data && response.data.id) {
                 ...
                 try {
                    ...
                    await axiosPrivate.post(`/payroll/tax-codes/${newId}/add-version/`, payload);
                    ...
                    alert("Tax Code created but failed to create initial version. Please add a version manually.");
                 }
             }
      }
...
const drawer= <>{isModalOpen && (
  ...
  const handleToggleVersion = async (taxCodeId, versionId, currentStatus) => {
    try {
      ...
      // Optimistic update
      setAllTaxCodes(prev => prev.map(c => {
         ...
         return {
            ...
            versions: c.versions.map(v => v.id === versionId ? { ...v, status: { ...v.status, active: newStatus } } : v)
         };
      }));

      // 1. Toggle Version
      await axiosPrivate.patch(`/payroll/tax-versions/${versionId}/`, {
         is_active: newStatus
      });
      ...
      if (isSingleVersion) {
         ...
         console.log(`[TaxCode] Single version toggled. Synced Parent ${taxCode.id} to ${newStatus}`);
      }
    ...
    <div className="bg-gray-50 dark:bg-slate-700 p-6 rounded shadow dark:shadow-black dark:inset-shadow-xs dark:inset-shadow-slate-600 flex flex-col md:flex-row items-center justify-between gap-4 transition-colors">
      ...
      <button
        onClick={() => {
          ...
          const baseData = last ? JSON.parse(JSON.stringify(last)) : {
            status: { active: true, locked: false },
            incomeTax: { type: 'progressive', flatRate: 0, brackets: [{ min: 0, max: '', rate: 0 }] },
            pension: { employeePercent: 0, employerPercent: 0 },
            statutoryDeductions: [],
          ...
          setTaxData({ ...baseData, version: `v${code.versions.length + 1}`, validFrom: '', validTo: null });
          openModal();
        }}
