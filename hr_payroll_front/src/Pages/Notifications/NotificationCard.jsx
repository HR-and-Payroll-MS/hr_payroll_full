...
export default function NotificationCard({ n, onView, onDelete, onMarkRead }) {
  return (
    <div
      onClick={() => {
        try {
          console.log('NotificationCard clicked (frontend):', n);
        } catch (e) {}
        onView && onView();
      }}
    ...
    >
      ...
      <div className="flex gap-4 items-center pl-2">
        {/* Icon Area */}
        <div
          className={`p-2 rounded ${n.unread ? 'bg-green-50 dark:bg-green-500/10' : 'opacity-60'}`}
        >
        ...
        <div className="flex-1">
          <div className="flex justify-between items-start">
            <h4
              className={`text-[11px] uppercase tracking-wider font-bold ${n.unread ? 'text-slate-800 dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}
            >
              {n.sender_view
                ? `To: ${n.recipient_name || 'Individual'}`
                : n.title}
          ...
          <div
            className={`text-sm mt-1 line-clamp-1 ${n.unread ? 'text-slate-600 dark:text-slate-300 font-medium' : 'text-slate-400 dark:text-slate-500 italic'}`}
          >
            {/* "{n.message}" */}
            <div
        ...
        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
          {n.unread && (
            <button
              onClick={(e) => {
                e.stopPropagation();
                try {
                  console.log('Notification markRead clicked (frontend):', n);
                } catch (e) {}
          ...
          <button
            onClick={(e) => {
              e.stopPropagation();
              try {
                console.log('Notification delete clicked (frontend):', n);
              } catch (e) {}
              onDelete && onDelete();
            }}
