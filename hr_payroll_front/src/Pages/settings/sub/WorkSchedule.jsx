...
export default function WorkSchedule() {
  ...
  const fetchSchedules = async () => {
    try {
      ...
      const mapped = rawData.map((s) => ({
        ...
        endTime: s.end_time,
        days:
          s.days_of_week && s.days_of_week.length
            ? s.days_of_week
            : [
  ...
  const handleSave = async () => {
    try {
      ...
      if (String(editingId).includes('new')) {
        ...
        // Update
        res = await axiosPrivate.put(
          `/attendances/schedules/${editingId}/`,
          payload,
        );
      }
  ...
  const handleCreateNew = () => {
    ...
    newSchedule.workingHoursDay = computeHoursPerDay(
  ...
  const handleBulkAssignSubmit = async () => {
    ...
    try {
      ...
      await axiosPrivate.post(
        `/attendances/schedules/${targetSchedule.id}/assign-bulk/`,
        payload,
      );
      ...
      <div className="flex sticky z-20 bg-white dark:bg-slate-800 p-4 top-0 gap-2 border-b border-gray-100 dark:border-slate-700 mb-6">
        ...
        <div className="flex items-center w-1/3 px-1.5 border border-gray-200 dark:border-slate-700 dark:bg-slate-900 rounded-lg">
          <div className="flex text-xs w-full items-center justify-between px-2.5 py-2.5 h-full">
            ...
            />
            <img
              className="h-3.5 opacity-45 dark:invert"
          ...
          (editingId ? (
            <div className="flex gap-2">
              <button
                ...
                className="flex bg-gray-200 text-gray-700 dark:bg-slate-700 dark:text-slate-200 items-center px-5 py-3 rounded-md font-semibold text-xs transition-transform active:scale-95"
              >
                Cancel
              </button>
              <button
                onClick={handleSave}
                className="flex bg-slate-800 text-white dark:bg-slate-100 dark:text-slate-900 items-center gap-1.5 px-5 py-3 rounded-md transition-transform active:scale-95"
              >
                <img
            ...
            <button
              ...
              className="flex bg-green-600 text-white items-center px-5 py-3 rounded-lg shadow-lg shadow-green-500/20 transition-transform active:scale-95"
            >
      ...
      <div className="flex flex-col gap-4">
        {filteredSchedules.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 bg-gray-50 dark:bg-slate-900/50 rounded-2xl border-2 border-dashed border-gray-200 dark:border-slate-800">
            <img
              src="/pic/empty-calendar.png"
              className="w-24 opacity-20 dark:invert mb-4"
              alt=""
            />
          ...
          filteredSchedules.map((sched) => {
            ...
            return (
              <div
                key={sched.id}
                className={`flex border ${isExpanded ? 'border-green-500 ring-2 ring-green-500/10' : 'border-gray-100 dark:border-slate-700'} rounded-xl bg-white dark:bg-slate-800 flex-col overflow-hidden transition-all shadow-sm`}
              >
                {/* TOP BAR */}
                <div
                  onClick={() => toggleExpand(sched.id)}
                  className={`flex p-5 flex-1 gap-5 justify-between items-center cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors`}
                >
                  <div className="flex-1 flex gap-3 items-center">
                    ...
                    {sched.isDefault && (
                      <p className="px-3 rounded-full text-[10px] py-1 text-green-600 dark:text-green-400 font-black bg-blue-50 dark:bg-blue-900/20 uppercase tracking-widest">
                        Default
                      </p>
                    )}
                  </div>
                  <div className="flex items-center gap-4">
                    ...
                    <img
                      className={`h-5 transition-transform dark:invert ${isExpanded ? 'rotate-0' : 'rotate-180 opacity-35'}`}
                      src="/svg/down-arrow-5-svgrepo-com.svg"
                      alt=""
                    />
                  </div>
                          ...
                          {isEditing ? (
                            <input
                              ...
                              onChange={(e) =>
                                setFormData({
                                  ...formData,
                                  workingHoursDay: e.target.value,
                                })
                              }
                            />
                          ) : (
                            <p className="text-sm font-bold text-slate-800 dark:text-slate-100">
                              {sched.workingHoursDay}
                            </p>
                          )}
                        </div>
                        <div className="flex justify-between items-center py-2 border-b border-slate-50 dark:border-slate-700/50">
                          ...
                          {isEditing ? (
                            <input
                              ...
                              onChange={(e) =>
                                setFormData({
                                  ...formData,
                                  totalHoursWeek: e.target.value,
                                })
                              }
                            />
                          ) : (
                            <p className="text-sm font-bold text-slate-800 dark:text-slate-100">
                              {sched.totalHoursWeek}
                            </p>
                          )}
                        </div>
                      </div>
                        ...
                        {isEditing ? (
                          <div className="space-y-6">
                            <div className="flex gap-6 p-4 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
                              <div className="flex-1">
                                ...
                                <input
                                  type="time"
                                  className="text-sm font-bold w-full outline-none bg-transparent dark:text-slate-100 border-b border-slate-200"
                                  ...
                                  onChange={(e) => {
                                    ...
                                    setFormData({
                                      ...formData,
                                      startTime: newStart,
                                      workingHoursDay: computeHoursPerDay(
                                        newStart,
                                        formData.endTime,
                                      ),
                                    });
                            ...
                            <div className="grid grid-cols-1 gap-2">
                              {formData.days.map((d, i) => (
                                <div
                                  key={i}
                                  className={`flex items-center gap-3 p-3 rounded border ${d.enabled !== false ? 'bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700' : 'bg-slate-100 dark:bg-slate-900 border-slate-200 dark:border-slate-800 opacity-50'}`}
                                >
                                  ...
                                  {d.enabled !== false && (
                                    <div className="flex items-center gap-2 flex-1">
                                      ...
                                      <input
                                        ...
                                        className="text-xs font-bold bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded px-2 py-1"
                                        value={
                                          d.customEndTime || formData.endTime
                                        }
                                        onChange={(e) => {
                                          const newDays = [...formData.days];
                                          newDays[i] = {
                          ...
                          <div className="space-y-2">
                            {sched.days.map((d, i) => (
                              <div
                                key={i}
                                className="flex justify-between items-center hover:bg-white dark:hover:bg-slate-800 p-2 rounded-lg transition-colors group"
                              >
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <p className="text-xs font-bold text-slate-500 group-hover:text-green-500 transition-colors tracking-tighter">
                                    {d.hours}
                                  </p>
                                </div>
                              </div>
      ...
      <Modal isOpen={isAssignModalOpen} location="center">
        <div className="bg-white dark:bg-slate-800 rounded-lg p-6 w-[500px] shadow-xl max-h-[80vh] flex flex-col">
          <h3 className="text-lg font-bold mb-4 dark:text-white">
            Assign Schedule: {targetSchedule?.name}
          </h3>
          ...
          <div className="flex-1 hover-bar overflow-y-auto mb-4 border rounded p-2 border-gray-100 dark:border-slate-700">
            {assignType === 'all' && (
              <p className="text-center text-gray-500 py-8">
                This will assign the schedule to ALL employees in the system.
              </p>
            )}
