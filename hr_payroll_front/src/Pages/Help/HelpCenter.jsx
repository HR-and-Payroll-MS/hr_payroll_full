...
export default function HelpCenter(){
    ...
    const middle = (
        <div id="middle" className="flex bg-white dark:bg-slate-800 w-full flex-col h-full p-2 gap-2 transition-colors"> 
            ...
            <div id="child">
                <NavLink to="/Employee/policies" className={({ isActive }) => `flex dark:hover:bg-slate-700 hover:bg-slate-50 rounded-md w-full justify-between items-center transition-all ${isActive ? "bg-slate-100 dark:bg-slate-700 shadow-sm" : "" }`}>
                    <div className="flex items-center gap-1.5 justify-start p-2 rounded">
                        <Icon name="FileText" size={18} className="opacity-40 dark:text-slate-200" />
                        <p className="font-bold text-gray-700 dark:text-slate-200 text-xs">Policies</p>
                    </div>
                </NavLink>
            </div>  
        </div>
