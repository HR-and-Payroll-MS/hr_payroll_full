    ...
    <div className="w-80 flex-shrink-0 bg-gray-50 dark:bg-slate-700 shadow dark:shadow-black dark:inset-shadow-xs dark:inset-shadow-slate-600 rounded-lg flex flex-col h-full overflow-hidden border-r border-gray-200 dark:border-slate-800">
      ...
      <div className="px-5 flex gap-6 text-sm font-medium pb-2">
        <button 
            ...
            className={`pb-2 ${filterType === 'All' ? 'text-emerald-400 border-b-2 border-emerald-400' : 'text-slate-400 hover:text-slate-200'}`}
        >
            All
        </button>
        <button 
            onClick={() => setFilterType('Unread')}
            className={`pb-2 ${filterType === 'Unread' ? 'text-emerald-400 border-b-2 border-emerald-400' : 'text-slate-400 hover:text-slate-200'}`}
        >
            Unread
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        {filteredUsers.map((user) => (
          <div 
            ...
            className={`flex items-center gap-3 p-4 cursor-pointer hover:bg-gray-100 dark:hover:bg-slate-600 transition-colors relative ${activeId === user.id ? 'bg-white dark:bg-slate-800 border-l-4 border-emerald-500 shadow-sm' : 'border-l-4 border-transparent'}`}
          >
            <div className="relative">
              <img 
              ...
              />
              {user.online && <div className="absolute bottom-0 right-0 w-3 h-3 bg-emerald-500 rounded-full border-2 border-white dark:border-slate-700"></div>}
            </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center justify-between">
                <p className="text-slate-400 text-xs truncate pr-2">{user.msg || user.role}</p>
                {/* Role Badge if msg is empty or just generic badge */}
                {user.role && <span className="text-[10px] px-1.5 py-0.5 rounded bg-slate-200 dark:bg-slate-600 text-slate-600 dark:text-slate-300">{user.role}</span>}
              </div>
            </div>
