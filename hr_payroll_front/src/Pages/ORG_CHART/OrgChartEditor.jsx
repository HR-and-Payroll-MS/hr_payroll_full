...
const OrgChartEditor = ({ initialData, userRole, onSave }) => {
  const canEdit =
    userRole === 'Manager' || userRole === 'HR' || userRole === 'HR_ADMIN';
  ...
  return (
    <div
      ref={containerRef}
      className="h-full w-full bg-slate-50 rounded-2xl relative shadow-inner overflow-hidden border border-slate-200"
    >
      <ReactFlow
        nodes={nodes}
        edges={edges}
      ...
      >
        ...
        <Panel
          ...
          className="bg-white/80 backdrop-blur-md p-2 rounded-2xl shadow-xl border border-white flex flex-col gap-1 mb-4 ml-4"
        >
          <button
            onClick={() => zoomIn()}
