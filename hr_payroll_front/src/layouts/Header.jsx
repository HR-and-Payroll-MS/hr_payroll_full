...
export default function Header() {
  ...
  const { unreadTotal, recomputeUnread } = useChatBadge();
  ...
  return (
    ...
    >
      <div
        id="left"
        className="flex py-2.5 w-2/5  justify-between items-center p-4 "
      >
        <div
          className={`flex relative shadow items-center gap-1.5 justify-start bg-gray-100 w-full h-full rounded-md  dark:bg-slate-700 `}
        >
          <div className="flex relative flex-1 items-center gap-1.5 py-2 h-full">
            <img
              className="h-4 left-2 absolute opacity-45"
            ...
            <InputField
              icon={false}
              border=""
              maxWidth="w-full pl-5"
              placeholder="Search..."
              searchMode="global"
              globalData={mainHeaderSearch[getLocalData('role')]}
              displayKey="label"
              onSelect={(item) => navigate(item.path)}
            />
          </div>
          <div
            className={`flex absolute right-2 bg-white items-center justify-center gap-1.5 px-1.5 rounded-md  dark:bg-slate-700 `}
          >
      ...
      <div id="middle" className="flex w-3/5 justify-start gap-7 items-center ">
        <p
          onClick={() => navigate('profile', { state: { position: 3 } })}
          className={`font-semibold text-gray-700  text-sm  dark:font-slate-300 dark:text-slate-300 hover:cursor-pointer hover:text-slate-900 `}
        >
          Documents
        </p>
        {role === 'Manager' ? (
          <p
            onClick={() => navigate('Announcement', { state: { position: 2 } })}
            className={`font-semibold text-gray-700  text-sm  dark:font-slate-300 dark:text-slate-300 hover:cursor-pointer hover:text-slate-900 `}
          >
            Announcements
          </p>
          ...
          <div onClick={handleNewsClick} className="relative cursor-pointer">
            <p
              className={`font-semibold text-gray-700  text-sm  dark:font-slate-300 dark:text-slate-300 hover:text-slate-900 `}
            >
              News
            </p>
            {unreadNewsCount > 0 && (
        ...
        <p
          onClick={() => navigate('profile', { state: { position: 2 } })}
          className={`font-semibold text-gray-700  text-sm  dark:font-slate-300 dark:text-slate-300 hover:cursor-pointer hover:text-slate-900 `}
        >
          Payslip
        </p>
      ...
      <div
        id="right"
        className="flex w-1/5 justify-end items-center px-6 gap-3"
      >
        <div
          onClick={() => {
            navigate('message');
            /* unread will be cleared per chat; recompute to reflect */ recomputeUnread?.();
          }}
          className="p-2 dark:hover:bg-slate-700 hover:bg-slate-100 rounded-full cursor-pointer transition-colors relative group"
        >
          <Icon
            name="MessageSquare"
            className="h-5 w-5 text-gray-600 dark:text-slate-300"
          />
          {unreadTotal > 0 && (
        ...
        <NotificationBell
          role={role}
          onOpenCenter={() => navigate('view_notification')}
        />
            ...
            <div className="flex items-center">
              {profile?.general?.photo ? (
                <img
                  ...
                  src={`${BASE_URL}${
                    profile.general.photo.startsWith('/') ? '' : '/'
                  }${profile.general.photo}`}
                  alt="Profile"
                  onError={(e) => {
        ...
        >
          <ul className="flex flex-col py-2">
            <li
              ...
              className="px-4 py-1 flex items-center gap-1.5 dark:hover:bg-slate-700 hover:bg-slate-50 cursor-pointer"
            >
              <Icon name="CircleUser" className="h-4 w-4" />
              view profile
            </li>
