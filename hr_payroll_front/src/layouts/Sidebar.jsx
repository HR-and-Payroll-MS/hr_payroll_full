...
export default function Sidebar() {
  ...
  const fetchLeaveCount = useCallback(async () => {
    if (!role) return;
    try {
      // Normalize roles from user role and groups
      ...
      for (const status of actionableStatuses) {
        ...
        const response = await axiosPrivate.get(
          `/leaves/requests/?status=${status}&_=${Date.now()}`
        );
        console.log('response.data thing -----------------', response.data);
  ...
  }, [role, auth?.user?.role, auth?.user?.groups, axiosPrivate]);

  // Listen for real-time leave updates (socket) and window events
  useSocketEvent('leave_updated', () => {
    fetchLeaveCount();
  });
  useEffect(() => {
    const handler = () => fetchLeaveCount();
    window.addEventListener('leave_updated', handler);
    window.addEventListener('focus', handler);
    const intervalId = setInterval(handler, 20000);
    return () => {
      window.removeEventListener('leave_updated', handler);
  ...
  const middle1 = (
    <div
      id="middle"
      className="flex relative flex-col w-full flex-1 my-4 hover-bar overflow-y-auto gap-2"
    >
        ...
        lists.path ? (
          ...
          <div key={index} className="cursor-pointer">
            <div
              ...
              className={` ${
                lists.Visible ? '' : 'hover:bg-slate-50 dark:hover:bg-slate-700'
              } flex w-full ${
              ...
              <div className="flex items-center gap-1.5 justify-center">
                <div className="relative">
                  <Icon
                    name={lists.Icons}
                    className={`w-4 h-4 ${
                      lists.Visible ? 'text-green-700' : 'text-slate-400'
                    }`}
                  />
                  {collapsed && lists.badge > 0 && (
                    <span className="absolute -top-1 -right-1 flex h-2 w-2">
            ...
            {!collapsed && (
              <div
                ...
                }  dark:border-slate-500 ml-4 my-1.5  border-l  relative rounded border-gray-300 flex-col gap-1.5`}
              >
                  ...
                  <>
                    {checking ? (
                      <div className="relative px-4.5">
                        ...
                        <div className="relative z-10 px-3 py-1 rounded bg-yellow-50 text-sm text-yellow-800">
                          Checking network...
                        </div>
                      </div>
                    ) : isLocal ? (
                      <div className="relative px-4.5 ">
                        <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[30%] h-2 border-b-1 border-slate-300 dark:border-slate-500 rounded-full z-0 " />
                        <NavLink
                          to={`/${clockinPath[role] || 'employee'}/clock_in`}
                          end
                          className={({ isActive }) =>
                            `relative z-10  flex rounded-md w-full justify-between items-center px-2.5 py-1.5 ${
                              isActive
                                ? 'bg-slate-200 shadow dark:bg-slate-700'
                                : 'hover:bg-slate-50 bg-white dark:bg-slate-800  dark:hover:bg-slate-700'
                            }`
                          }
                        >
                          <p className="dark:text-slate-300 font-semibold text-gray-700 text-sm">
                      ...
                      <div className="relative px-4.5 ">
                        ...
                        <div className="relative z-10 px-3 bg-white dark:bg-slate-800  py-1 hover:cursor-not-allowed rounded text-sm text-gray-400">
                          Clock In
                        </div>
                      </div>
                    )}
                    <div
                      className={`px-5 flex gap-1 items-center  text-xs font-light ${
                        checking
                          ? 'text-gray-400'
                          : isLocal
                          ? 'text-green-500'
      ...
      {!collapsed && (
        <>
          ...
          <div className="relative dark:bg-slate-900  inset-shadow-slate-200 inset-shadow-2xs shadow shadow-slate-100 dark:inset-shadow-xs dark:inset-shadow-black dark:shadow dark:shadow-slate-700 flex gap-0.5 rounded-4xl cursor-pointer bg-gray-50 h-11 w-full justify-around items-center m-0.5 p-1 overflow-hidden">
            ...
            <div
              className={`absolute top-1 bottom-1 w-[48%] rounded-4xl transition-all duration-500 ease-[cubic-bezier(0.175,0.885,0.32,1.275)] bg-white dark:bg-slate-800 shadow-md ${
            ...
            <div
              ...
              className={`relative z-10 flex flex-1 h-full items-center gap-1.5 justify-center py-2.5 transition-colors duration-300 ${
                theme === 'light' ? 'text-gray-800' : 'text-slate-400'
              }`}
            >
