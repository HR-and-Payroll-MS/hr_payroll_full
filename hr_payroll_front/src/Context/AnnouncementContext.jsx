...
export const AnnouncementProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const handleDeletePost = (postId) => {
      setAnnouncements((prev) =>
        Array.isArray(prev)
          ? prev.filter((a) => a._id !== postId && a.id !== postId)
          : []
      );
    };

    const handleViewUpdate = ({ id, views }) => {
      setAnnouncements((prev) =>
        prev.map((a) =>
    ...
    socketObj.on('announcement_view_update', handleViewUpdate);

    return () => {
      socketObj.off('new_announcement', handleNewPost);
      socketObj.off('delete_announcement', handleDeletePost);
      socketObj.off('announcement_view_update', handleViewUpdate);
    };
  }, [socketObj]);
