...
const EmployeePayslipTemplate = ({ payroll }) => {
  ...
  // Use provided payroll data if available, otherwise fall back to mockData
  const sourcePayroll = payroll || mockData;
  if (!sourcePayroll) return null;

  const source = sourcePayroll.details || sourcePayroll;

  // Normalize employee fields coming from different payload shapes
  const employee = {
    id:
      sourcePayroll.employee_id_display ||
      sourcePayroll.id ||
      (sourcePayroll.employee &&
        (sourcePayroll.employee.employee_id || sourcePayroll.employee.id)) ||
      'N/A',
    name:
      sourcePayroll.employee_name ||
      sourcePayroll.name ||
      (sourcePayroll.employee &&
        (sourcePayroll.employee.fullname || sourcePayroll.employee.name)) ||
      'Employee',
    department:
      sourcePayroll.department ||
      (sourcePayroll.employee &&
        (sourcePayroll.employee.department ||
          sourcePayroll.employee.department?.name)) ||
      (source.company && source.company.department) ||
      '',
    jobTitle:
      sourcePayroll.job_title ||
      sourcePayroll.jobTitle ||
      (sourcePayroll.employee &&
        (sourcePayroll.employee.job_title ||
          sourcePayroll.employee.jobTitle)) ||
      '',
    bankAccount:
      sourcePayroll.bank_account ||
      sourcePayroll.bankAccount ||
      (sourcePayroll.employee &&
        (sourcePayroll.employee.bank_account ||
          sourcePayroll.employee.bankAccount)) ||
      '',
  };
    ...
    >
      ...
      >
        ...
        >
          ...
          <div style={{ textAlign: 'right' }}>
            <h2 style={{ fontSize: '1.125rem', margin: 0, color: '#1e293b' }}>
              PAY ADVICE
            </h2>
            <div
              style={{ fontSize: '1rem', fontWeight: 700, color: '#4f46e5' }}
            >
              {source.month}
            </div>
          </div>
        ...
        >
          ...
          <div
            style={{ borderLeft: '1px solid #e2e8f0', paddingLeft: '1.5rem' }}
          >
            <h3
              style={{
                fontSize: '0.7rem',
            ...
            <div style={{ fontSize: '0.85rem' }}>
              ...
              {(() => {
                const raw = employee.bankAccount || '';
                const parts = raw.split(' ');
                const account = parts.length > 0 ? parts.pop() : '';
              ...
              <div style={{ marginTop: '6px' }}>Date: {source.paymentDate}</div>
              <div>Method: {source.paymentMethod}</div>
            </div>
          </div>
        </div>
        ...
        >
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              ...
              >
                <th style={{ paddingBottom: '10px' }}>EARNINGS</th>
                <th style={{ paddingBottom: '10px', textAlign: 'right' }}>
                  AMOUNT
                </th>
              </tr>
            </thead>
        ...
        >
          <div>
            ...
            <div style={{ fontSize: '1.25rem', fontWeight: 600 }}>
              {source.gross.toLocaleString()}
            </div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div
              style={{ fontSize: '0.85rem', color: '#4ade80', fontWeight: 700 }}
            >
              NET PAYABLE
            </div>
            <div
