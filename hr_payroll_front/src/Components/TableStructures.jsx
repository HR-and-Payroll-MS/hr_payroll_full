  ...
  const deleteItem = () => {
    ...
    switch (id) {
      case 1:
        ...
        const isStatus = typeof value === 'string' && ['PRESENT', 'LATE', 'ABSENT', 'PERMISSION', 'LEAVE'].includes(value.toUpperCase());
        
        let colorClass = "text-gray-700 dark:text-slate-300";
        if (isStatus) {
           const upper = value.toUpperCase();
           if (upper === 'PRESENT') colorClass = "text-emerald-600 dark:text-emerald-400 font-bold";
           else if (upper === 'ABSENT') colorClass = "text-red-500 dark:text-red-400 font-bold";
           else if (upper === 'LATE') colorClass = "text-amber-500 dark:text-amber-400 font-bold";
           else if (upper === 'PERMISSION') colorClass = "text-blue-500 dark:text-blue-400 font-bold";
        }
          ...
          <div className="flex w-full justify-start items-center gap-2">
            {item[0] ? (
              <img 
                ...
                src={item[0].startsWith('http') ? item[0] : `${BASE_URL}${item[0]}`}
                alt={item[1]}
                onError={(e) => { 
            ...
            >
              {(item[1] ?? "").split(" ").map(n => n[0]).slice(0, 2).join("") || "NA"}
            </div>
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div className="flex flex-col">
                                   <div className="flex items-center gap-1">
                                      {data?.hasIssues && (
                                        <span title={data?.issueNotes || "Calculation issue"} className="cursor-help text-sm">⚠️</span>
                                      )}
      ...
      case 81:
        const dateVal = item[0] ? new Date(item[0]).toISOString().split('T')[0] : "-";
        return (
      ...
      case 82:
        ...
        const displayShortName = fWords.length > 2 
          ? fWords.slice(0, 2).join(" ") + "..." + fExtension
          : fullFilename;

        return (
          <div className="flex w-full justify-start items-center dark:text-slate-300">
            <p className="font-semibold text-sm" title={fullFilename}>{displayShortName}</p>
          </div>
        );
      case 83:
        const fullNotes = item[0] || "";
        const nWords = fullNotes.split(/\s+/).filter(Boolean);
        const displayShortNotes = nWords.length > 2 
          ? nWords.slice(0, 2).join(" ") + "..."
          : fullNotes;
        return (
          <div className="flex w-full justify-start items-center dark:text-slate-300">
            <p className="text-xs" title={fullNotes}>{displayShortNotes || "-"}</p>
          </div>
        );
      default:
        return <p>-</p>;
