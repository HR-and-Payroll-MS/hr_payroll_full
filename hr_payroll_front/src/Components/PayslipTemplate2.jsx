...
const PayslipTemplate2 = ({ payroll, editable = false, onSave }) => {
  ...
  useEffect(() => {
    if (payroll) {
      const cloned = JSON.parse(JSON.stringify(payroll));
      setFormData(cloned); // Deep clone to avoid mutation
      setOriginal(cloned);
      setSaveMsg('');
    }
  }, [payroll]);
  ...
  const handleSave = async () => {
    try {
      ...
      const updated = res?.data?.payslip || formData;
  ...
  return (
    ...
    >
        ...
        <div style={{ display: 'flex', gap: '1rem' }}>
          ...
          <div>
            <h1 style={{ fontSize: '1.25rem', fontWeight: 700, margin: 0 }}>
              {source.company?.name || 'Company Name'}
            </h1>
      ...
      >
        <div>
          ...
          <div style={{ fontSize: '0.85rem', fontWeight: 600 }}>
            ...
            <span style={{ color: '#4f46e5' }}>
              {formData.taxCode || 'Not Configured'}
            </span>
            <span
              style={{
                marginLeft: '8px',
        ...
        <div style={{ borderLeft: '2px solid #e2e8f0', paddingLeft: '15px' }}>
          ...
          <div style={{ fontSize: '0.85rem' }}>
            Days Attended: <b>{formData.attendedDays || 0}</b> | LOP Days:{' '}
            <b style={{ color: '#ef4444' }}>{formData.lopDays || 0}</b>
          </div>
        </div>
      </div>
        ...
        <div>
          ...
          >
            <tbody>
              ...
              <tr>
                <td style={{ padding: '4px 0', color: '#64748b' }}>ID:</td>
                <td>{employee?.id}</td>
              </tr>
              <tr>
          ...
          >
            <tbody>
              <tr>
                ...
                <td>{source.paymentMethod || '-'}</td>
              </tr>
              <tr>
                <td style={{ padding: '4px 0', color: '#64748b' }}>Date:</td>
                <td>{source.paymentDate || '-'}</td>
              </tr>
              <tr>
                <td style={{ padding: '4px 0', color: '#64748b' }}>
                  Bank Acc:
          ...
          <table style={{ width: '100%', fontSize: '0.875rem' }}>
            <tbody>
              ...
              <tr style={{ borderTop: '1px solid #e2e8f0', fontWeight: 700 }}>
                ...
                <td style={{ textAlign: 'right', padding: '8px 0 0 0' }}>
                  {(source.gross || 0).toLocaleString()}
                </td>
              </tr>
            </tbody>
          </table>
          ...
          <table style={{ width: '100%', fontSize: '0.875rem' }}>
            <tbody>
              ...
              <tr style={{ borderTop: '1px solid #e2e8f0', fontWeight: 700 }}>
                ...
                <td style={{ textAlign: 'right', padding: '8px 0 0 0' }}>
                  {(source.totalDeductions || 0).toLocaleString()}
                </td>
              </tr>
            </tbody>
          </table>
      ...
      >
        <strong>Policy Note:</strong> Calculations include statutory tax
        compliance and internal conduct policies. Any adjustments to Attendance
        or Conduct deductions must be approved by the Department Head.
      </div>
      ...
      >
        ...
        >
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <div
              style={{ fontSize: '1.75rem', fontWeight: 700, color: '#4ade80' }}
            >
