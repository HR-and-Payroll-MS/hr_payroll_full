...
const StepJob = ({ data , onChange }) => {
    ...
    const getManagersForDepartment = useMemo(() => {
        return (deptId) => {
            ...
            if (dept.manager) {
                ...
                const managerEmployee = employees.data?.find(e => e.id === dept.manager);
                if (managerEmployee) {
                    managers.push({
            ...
            const deptEmployees = employees.data?.filter(e => 
                e.department === dept.name && 
                (e.jobtitle === 'Department Manager' || e.job?.jobtitle === 'Department Manager')
            ) || [];
    ...
    const handleDeptChange = (deptName) => {
        ...
        if (selected) {
            onChange({ 
             ...
             onChange({ department: '', departmentName: '', linemanager: '', linemanagerName: '' });
        }
    };
    ...
    const handleLineManagerChange = (managerName) => {
        ...
        if (selected) {
            onChange({ linemanager: selected.id, linemanagerName: selected.fullname });
        }
    };
    
    const deptOptions = departments.data ? departments.data.map(d => d.name) : [];
        ...
        <div className="bg-white dark:bg-slate-800 rounded shadow dark:shadow-black dark:inset-shadow-xs dark:inset-shadow-slate-600 transition-all mb-8 border border-slate-100 dark:border-transparent">
            ...
            <div id="left" className="flex gap-5 p-4 justify-start items-start flex-wrap">
                <div className="w-96 flex gap-2 text-nowrap items-center">
                    ...
                    <input type="text" disabled placeholder="(Auto-generated)" value={data.employeeid} onChange={(e) => onChange({ employeeid: e.target.value })} className="w-full bg-slate-100 dark:bg-slate-900 cursor-not-allowed text-sm text-slate-500 border border-slate-200 dark:border-slate-700 rounded px-3 py-1.5 outline-none shadow-sm" />   
                </div>
                <div className="w-96 flex gap-2 text-nowrap items-center">
        ...
        <div className="bg-white dark:bg-slate-800 rounded shadow dark:shadow-black dark:inset-shadow-xs dark:inset-shadow-slate-600 transition-all mb-8 border border-slate-100 dark:border-transparent">
            ...
            <div id="left" className="flex gap-5 p-4 justify-start items-start flex-wrap">
                ...
                <div className="w-96 flex gap-2 text-nowrap items-center">
                    ...
                    <div className="w-full">
                        {!data.department ? (
                            <p className="text-sm text-slate-400 italic px-3 py-1.5">Select a department first</p>
                        ) : autoAssignedManager ? (
                            ...
                            <p className="text-sm text-amber-500 italic px-3 py-1.5">No managers in this department</p>
                        )}
                    </div>
                </div>
            </div>
    ...
    const handleScheduleChange = (scheduleTitle) => {
        ...
        if (selected) {
            onChange({ workschedule: selected.id, workscheduleTitle: selected.title });
        }
    };
    ...
    const workSchedule = (
        <div className="bg-white dark:bg-slate-800 rounded shadow dark:shadow-black dark:inset-shadow-xs dark:inset-shadow-slate-600 transition-all border border-slate-100 dark:border-transparent">
            ...
            <div className="p-4 flex justify-start items-start">
                 <div className="w-96 flex gap-2 text-nowrap items-center">
                    <p className="min-w-40 text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-tighter">Schedule</p>
                    <div className="w-full">
                        <Dropdown 
                            padding='p-1.5' 
